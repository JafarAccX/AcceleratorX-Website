import { e as createLucideIcon, r as reactExports, u as useNavigate, z as zt, j as jsxRuntimeExports, S as SEO, l as LogOut, k as authService } from "./main-DUtAh55Z.js";
import { c as createClient } from "./index-Bq44Z0X_.js";
import { P as Plus } from "./plus-BLnmhFOd.js";
...
const blogService = {
  ...
  async createBlog(blog) {
    const { data, error } = await supabase.from("blogs").insert([transformBlogForDB(blog)]).select().single();
    if (error) throw error;
    return data;
  },
  async updateBlog(id, blog) {
    const { data, error } = await supabase.from("blogs").update(transformBlogForDB(blog)).eq("id", id).select().single();
    if (error) throw error;
    return data;
  },
  async deleteBlog(id) {
...
function BlogDashboard() {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      zt.error("Failed to fetch blogs");
    } finally {
      setLoading(false);
    }
  };
  const handleDelete = async (blog) => {
    if (window.confirm("Are you sure you want to delete this blog?")) {
      try {
        ...
        zt.success("Blog deleted successfully");
      } catch (error) {
        zt.error("Failed to delete blog");
      }
    }
  };
