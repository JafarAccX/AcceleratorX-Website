import { r as reactExports, j as jsxRuntimeExports, u as useNavigate, b as useUser, L as Link, c as _t, d as api } from "./main-BP2-o3HA.js";
...
const SignUpForm = () => {
  ...
  const handleSendOTP = async () => {
    if (!formState.firstName || !formState.lastName || !formState.email) {
      _t.error("Please fill in your name and email first.");
      return;
    }
  ...
  const handleRegister = async () => {
    if (!otp || otp.length !== 6) {
      _t.error("Please enter a valid 6-digit OTP.");
      return;
    }
    setIsProcessing(true);
    try {
      ...
      if (response.status === 201 && response.data.success) {
        ...
        _t.success("Registration successful! Welcome.");
        navigate("/profile");
      } else {
        throw new Error(
