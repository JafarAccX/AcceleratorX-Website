import { d as useParams, u as useNavigate, b as useUser, r as reactExports, j as jsxRuntimeExports, z as zt } from "./main-DS-AYwzb.js";
...
function JobApplication() {
  ...
  const validateForm = () => {
    if (!isValidUrl(formData.resumeUrl)) {
      zt.error("Please provide a valid resume URL");
      return false;
    }
    if (formData.portfolioUrl && !isValidUrl(formData.portfolioUrl)) {
      zt.error("Please provide a valid portfolio URL or leave it empty");
      return false;
    }
    if (formData.linkedinUrl && !isValidUrl(formData.linkedinUrl)) {
      zt.error("Please provide a valid LinkedIn URL or leave it empty");
      return false;
    }
    if (formData.githubUrl && !isValidUrl(formData.githubUrl)) {
      zt.error("Please provide a valid GitHub URL or leave it empty");
      return false;
    }
    return true;
  };
  const handleSubmit = async (e) => {
    ...
    try {
      if (!user || !job) {
        zt.error("Please login to apply for this job");
        setSubmitting(false);
        return;
      }
      const hasApplied = await jobApplicationService.hasUserApplied(user.id, id || "");
      if (hasApplied) {
        zt.error("You have already applied to this job!");
        setSubmitting(false);
        return;
      }
      if (!validateForm()) {
      ...
      zt.error("Failed to submit application. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
