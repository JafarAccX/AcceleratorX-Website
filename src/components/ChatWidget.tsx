...
export default function ChatWidget() {
  ...
  const updateQueryHistory = async (query: string) => {
    ...
    try {
      ...
      const { data: userData } = await supabase.from("chat_users").select("query_history").eq("id", userId).single();

      // Prepare the new query entry
      const newQuery = { query, timestamp };

      // Create or update the query history array
      const updatedHistory = userData?.query_history ? [...userData.query_history, newQuery] : [newQuery];
  ...
  const handleUserInput = async (input: string) => {
    ...
    if (!nameRequested && !userInfo.name) {
      ...
      addMessage(
        "bot",
        "Thanks for reaching out! I'm your AcceleratorX assistant. Could you please tell me your name so I can better assist you?",
      );
      return;
    }
    ...
    if (nameRequested && !userInfo.name) {
      ...
      addMessage(
        "bot",
        `Nice to meet you, ${input}! Could you please share your phone number so I can better assist you?`,
      );
      return;
    }
    ...
    if (!userInfo.phone) {
      ...
      if (!phoneRegex.test(input)) {
        ...
        addMessage(
          "bot",
          "Please provide a valid phone number. It should be between 7-15 digits and may include spaces, +, -, or parentheses.",
        );
        return;
      }
      ...
      try {
        ...
        const { data, error } = await supabase.from("chat_users").insert([userData]).select();
      ...
      addMessage(
        "bot",
        `Thank you, ${userInfo.name}! I'm here to help with any questions about AcceleratorX's programs, pricing, application process, or anything else you'd like to know. How can I assist you today?`,
      );
      return;
    }
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-6 z-50 w-[400px] max-w-[calc(100vw-2rem)]">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-800 flex flex-col h-[600px] max-h-[80vh] transition-all duration-300 ease-in-out">
            ...
            <div
              className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-gray-950 scroll-smooth"
