...
const initializePixel = (pixelId: string) => {
  ...
  // to avoid "[Meta Pixel] - Duplicate Pixel ID" warnings
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const MetaPixel = () => {
  ...
  useEffect(() => {
    if (!isClient || !pixelId) {
      if (!pixelId && import.meta.env.DEV) {
    ...
    const checkFbq = () => {
      ...
      // Track events (only if URL has changed to prevent duplicates)
      const currentFullUrl = location.pathname + location.search;
      if (lastTrackedUrlRef.current === currentFullUrl) return;
      
      if (isPageViewRoute) {
        if (import.meta.env.DEV) console.log(`Tracking PageView for pixel: ${pixelId} on ${currentFullUrl}`);
        window.fbq('track', 'PageView');
        lastTrackedUrlRef.current = currentFullUrl;
      }

      if (isLeadRoute) {
        if (import.meta.env.DEV) console.log(`Tracking Lead for pixel: ${pixelId}`);
        window.fbq('track', 'Lead');
      }

      if (isCompleteRegistrationRoute) {
        if (import.meta.env.DEV) console.log(`Tracking CompleteRegistration for pixel: ${pixelId}`);
